...
function setNavText () {
  if (scrollPos >= (stepWidth * (stepsArray.length -1))) {
    // If we're on the last step, set next nav text to a green checkmark
    ...
    // This seems redundent to do ever time we're not on the first or last
    ...
    // Double chek that the next nav text is correct and set the proper color
    document.querySelector('.step-nav.next span').textContent = '»'
